...
Generate_Cash_Flows = function( lease_df_input ){
  ...
  if( tolower( lease_df_input$`Lease.Payable.At` ) == 'end' ){
    
    cashflow_df$Date = LastDayInMonth( cashflow_df$Date )
    
  } else{ cashflow_df$Date = lubridate::floor_date( cashflow_df$Date, 'month' ) }
  
  first_row = data.table( LeaseId = lease_df_input$`Lease.ID`, Date = as.Date( lease_df_input$`Initial.Application.Date`, format="%d-%m-%Y" ),
                          
                          Type = 'Initial direct costs', 'Lease_Rental' = 0, 'Upfront_Cost' = lease_df_input$`Upfront.Payments` ) %>% as.data.frame()
  
  cashflow_df_1 = bind_rows( first_row, cashflow_df )
  
  #..... Dismantling costs and residual guarantee rows ....
  
  dismantling_df = data.table( LeaseId = lease_df_input$`Lease.ID`, Date = rep( as.Date( lease_df_input$`Date.of.dismantling`, format="%d-%m-%Y" ), 2 ),
                               
                               Type = c( 'Dismantling costs', 'Residual Value Guarantee' ), 
                               
                               'Lease_Rental' = c( lease_df_input$`Dismantling.Cost`, lease_df_input$`Residual.Guarantee.Payments` ), 
  ...
  cashflow_df_2[["Discount_Factor"]] = exp( -( lease_df_input$`Incremental.Borrowing.Rate`/(100*365) )*( as.integer( cashflow_df_2$Date - cashflow_df_2$Date[1] ) ) )
  
  cashflow_df_2[["Present_Value"]] = round( cashflow_df_2$Total*cashflow_df_2[["Discount_Factor"]] )
  
  cashflow_df_2$Amortization = round( c( NA, diff( cashflow_df_2$Date ) )/as.integer( ( as.Date( lease_df_input$`End.Date`, format="%d-%m-%Y" ) - as.Date( lease_df_input$`Rent.Effective.Date`, format="%d-%m-%Y" ) ) )*sum( cashflow_df_2[["Present_Value"]] ) )
  
  cashflow_df_2 = cashflow_df_2 %>% mutate( 'Debit_Account' = 'Lease_Liability', 'Credit_Account' = 'Bank_Account', Amout = Total )
  
  #.... Total row ....
  
  total_df = data.table( LeaseId = 'Total', Date = as.Date( lease_df_input$`End.Date`, format="%d-%m-%Y" ), Type = '', 
                         
                         'Lease_Rental' = sum( cashflow_df_2$`Lease_Rental` ), 'Upfront_Cost' = lease_df_input$`Upfront.Payments`, 
...
Generate_Lease_Deposit = function( my_advances_df_input, lease_df_input ){
  ...
  if( ( nrow( lease_df_input ) > 0 ) & ( nrow( my_advances_df_input ) > 0 ) ){
    ...
    lease_deposit_df = data.table( LeaseId = lease_df_input$`Lease.ID`, 
                              
                              Date = seq.Date( as.Date( lease_df_input$`Rent.Effective.Date`, format="%d-%m-%Y" ),
                                               
                                               as.Date( lease_df_input$`End.Date`, format="%d-%m-%Y" ), lease_payment_freq ), 'Lease Deposit' = -1 ) 
    ...
    lease_deposit_df = lease_deposit_df %>%
      ...
      mutate( 'Installment' = 1:nrow( lease_deposit_df ),
              ...
              'Discount Factor' = exp( -( ( my_advances_df_input$Risk.Free.Rate + my_advances_df_input$Credit.Spread )/( 100*365 ) )*( as.integer( ( Date - as.Date( lease_df_input$`Rent.Effective.Date`, format="%d-%m-%Y" ) ) ) ) ), 
    ...
    first_row = data.table( LeaseId = lease_df_input$`Lease.ID`, Date = as.Date( lease_df_input$`Initial.Application.Date`, format="%d-%m-%Y" ),
                            ...
                            Installment = 0, 'Lease Deposit' = as.numeric( gsub( ',', '',  my_advances_df_input$Lease.Advance ) ) ) %>%
                            
                            mutate( 'Discount Factor' = exp( -( ( my_advances_df_input$Risk.Free.Rate + my_advances_df_input$Credit.Spread )/( 100*365 ) )*( as.integer( ( tail( lease_deposit_df$Date, 1 ) - as.Date( lease_df_input$`Rent.Effective.Date`, format="%d-%m-%Y" ) ) ) ) ) ) %>%
    ...
    for( i in 1:nrow( lease_deposit_df_1 ) ){
      ...
      if( i == 1 ){ car_val_deposit[i] = lease_deposit_df_1$`Carrying value of deposit`[1] } else{
        ...
        car_val_deposit[i] = car_val_deposit[i-1]*exp( ( ( my_advances_df_input$Risk.Free.Rate + my_advances_df_input$Credit.Spread )/( 100*365 ) )*lease_deposit_df_1$date_diff[i] )
